...
VS Code extension for Harbour and xHarbour, forked from
[harbourCodeExtension](https://github.com/APerricone/harbourCodeExtension)
by Antonino Perricone. See `NOTICE.md` (repo root) for attribution and
`LICENSE` (GPL-3.0-or-later) for licensing. If you like the original
project, consider supporting Antonino Perricone directly — see his
repository for donation links.

## What's new in this fork

- `harbour.aliases.customKeywords`: paint your own PP-macro keywords
  (e.g. a `#command DEFAULT <v> := <x> => Default(<v>, <x>)` alias) with
  keyword coloring, configurable per word.
- `harbour.aliases.callSuffixes`: treat `identifier:Suffix(...)` (e.g.
  `minhafuncao:Exec()`) as a call to `identifier` for signature help and
  completion.
- Hover now shows the comment block immediately above a function/procedure/
  method, in addition to the existing `#define` hover.
